    ...
    <nav className="bg-[#FFFDF9] dark:bg-[#1f2937] fixed !w-full top-0 z-50 shadow-md transition-colors duration-300">
      ...
      {isOpen && (
        <>
          {/* Overlay */}
          <div
            className="fixed inset-0 bg-black/90 dark:bg-black/90 z-40 md:hidden lg:hidden"
            onClick={() => setIsOpen(false)}
          ></div>

          {/* Sliding Menu */}
          <div
            className={`fixed top-0 left-0 w-60 h-full bg-[#FFFDF9] dark:bg-[#263548] z-50 transform transition-transform duration-200 !ease-in-out flex flex-col justify-between !p-6 md:hidden lg:hidden ${
          ...
          >
            ...
            {/* Menu Links */}
            <div className="flex flex-col !gap-8 text-gray-800 dark:text-gray-200">
              <a
                href="#projects"
                onClick={() => setIsOpen(false)}
                className="flex items-center gap-2 hover:text-cyan-500 font-medium transition border-b border-gray-300 dark:border-gray-700 !pb-4"
              >
                <FaReact /> Projects
              </a>
              <a
